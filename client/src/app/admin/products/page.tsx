...
// Component ProductModal (dùng để thêm/sửa sản phẩm)
...
// Component chính: ProductManagement (Admin Page)
          ...
          onSave={(product) => {
            if (currentProduct) {
              setProducts(
                products.map((p) =>
                  p.id === product.id
                    ? { ...product, id: p.id, image: product.image || "" }
                    : p
                )
              );
            } else {
              setProducts([
                ...products.map((p) => ({ ...p, image: p.image || "" })),
                {
                  ...product,
                  id: products.length
